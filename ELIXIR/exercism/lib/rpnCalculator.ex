defmodule RPNCalculator do
  @moduledoc """
  Errors
  Errors happen. In Elixir, while people often say to "let it crash",
  there are times when we need to rescue the function call to a known good state to fulfill a software contract.
  In some languages, errors are used as a method of control flow, but in Elixir, this pattern is discouraged.
  We can often recognize functions that may raise an error just by their name: functions that raise errors are to have ! at the end of their name.
  This is in comparison with functions that return {:ok, value} or :error. Look at these library examples:
  ...
  Line 4, in the rescue section, we pattern match on the Module name of the error raised
  on the left side of ->:
  ...
  """
  @spec calculate!(list(), (list() -> any())) :: any()
  ...
  @spec calculate(list(), (list() -> any())) :: {:ok, any()} | :error
  def calculate(stack, operation) do
    try do
      {:ok, operation.(stack)}
    rescue
  ...
  @spec calculate_verbose(list(), (list() -> any())) :: {:ok, any()} | {:error, String.t()}
